...
class AACertificateController extends Controller
{
    ...
    public function view($course_code)
    {
        ...
        //count
        $count = StudentApplicant::where('certificate_status', '1')->where('status', '1')->where('award_applied', '1')->where('school_year', getAcademicYear())->count();
        $total = StudentApplicant::where('status', '1')->where('award_applied', '1')->where('course_id', $courses->id)->where('school_year', getAcademicYear())->count();
    ...
    public function showCertificate($course_code, $id)
    {
        ...
        $qrcode = base64_encode(QrCode::format('svg')->color(128, 0, 0)->size(300)->errorCorrection('H')->generate(shortUrl() . '/verify-award/' . $stud->stud_app_id));
