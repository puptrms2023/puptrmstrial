...
@section('content')
    <div class="modal" tabindex="-1" id="deleteModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ url('admin/non-academic-award/outside-competition/delete-form') }}" method="POST">
                    @csrf
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div class="h3 mb-0 text-gray-800">Awards Received from outside competition</div>
    </div>
                        ...
                        <tbody>
                            @foreach ($form as $award)
                                <tr>
                                    ...
                                    <td>{{ $award->orgs->name }}</td>
                                    <td>{{ $award->competition_name }}</td>
                                    <td>{{ $award->placements }}</td>
                                    <td>
                                        <img src="{{ asset('uploads/' . $award->image) }}" class="img-thumbnail img-circle"
                                            width="50" alt="Image">
                                    </td>
                                    <td>
                                        <a href="{{ url('admin/non-academic-award/outside-competition/' . $award->id) }}"
