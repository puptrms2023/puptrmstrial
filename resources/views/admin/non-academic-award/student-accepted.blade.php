...
<head>
    <title>Accepted Applicants - {{ $nonacad->name }}</title>
    <style type="text/css">
        body {
...
<body>
    ...
    <table class="table3">
        <tr>
            <th colspan="2" style="text-align: left; font-size:22px;font-weight:bold;">
                List of Accepted Applicants - {{ $nonacad->name }}</th>
        ...
        <body>
            @foreach ($students as $stud)
                <tr>
                    ...
                    <td>
                        @if ($stud->nonacad_id == '1')
                            <P>School Organization: {{ $stud->orgs->name }}
                                @if (!empty($stud->others))
                                    - {{ $stud->others }}
                                @endif
                            </p>
                        @elseif ($stud->nonacad_id == '2')
                            <P>Sport: {{ $stud->sports }}</p>
                        @elseif ($stud->nonacad_id == '3')
                            <P>School Organization: {{ $stud->orgs->name }}
                                @if (!empty($stud->others))
                                    - {{ $stud->others }}
                                @endif
                            </p>
                        @elseif ($stud->nonacad_id == '4')
                            <P>Subject Name: {{ $stud->subject_name }}<br>
                                Thesis Title: {{ $stud->thesis_title }}</p>
                        @elseif ($stud->nonacad_id == '5')
                            <P>School Organization: {{ $stud->orgs->name }}
                                @if (!empty($stud->others))
                                    - {{ $stud->others }}
                                @endif
                            </p>
                        @elseif ($stud->nonacad_id == '6')
                            <P>Designation Office: {{ $stud->designated_office }}<br>
                                School Organization: {{ $stud->orgs->name }}
                                @if (!empty($stud->others))
                                    - {{ $stud->others }}
                                @endif
                            </p>
                        @elseif ($stud->nonacad_id == '7')
                            <P>Competition Name: {{ $stud->competition_name }}<br>
                                Placements: {{ $stud->placement }}<br>
                                School Organization: {{ $stud->orgs->name }}
                                @if (!empty($stud->others))
                                    - {{ $stud->others }}
                                @endif
